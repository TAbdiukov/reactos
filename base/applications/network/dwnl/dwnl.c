...
CBindStatusCallback_UpdateProgress(CBindStatusCallback *This)
{
    ...
    if (This->Size != 0)
    {
        ...
        if (Percentage > 200) // If percentage is greater than 200%, do a failsafe
            Percentage = 99;
                               ...
                               LPCWSTR szStatusText)
{
    ...
    switch (ulStatusCode)
    {
        ...
        case BINDSTATUS_ENDDOWNLOADDATA:
			/* since download is completed, update progress one last time to be at 200% */
            if(This->Size) // To avoid division by zero: if file size is known
            {

                This->Progress = (This->Size)*2; // Ensure progress == total size
                CBindStatusCallback_UpdateProgress(This); // Show 200% progress now
            }
